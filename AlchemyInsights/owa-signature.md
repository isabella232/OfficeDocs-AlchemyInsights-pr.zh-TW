---
title: 在 Outlook 網頁版中建立簽章
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: fb90175028c65c43468473885926b3e7ac9eceb0
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/22/2020
ms.locfileid: "43768392"
---
# <a name="create-an-email-signature-in-outlook-on-the-web"></a>在 web 上的 Outlook 中建立電子郵件簽名

1. 跳至 <img src='data:image/png;base64,' />
 **設定** > 會**顯示所有 Outlook 設定** > **撰寫及回復**。
1. 在 [**電子郵件簽名**] 底下，輸入您的簽章並使用可用的格式設定選項，以變更其外觀。

*附注：* 每個帳戶只能有一個簽名。

若要深入瞭解，請參閱[在 Outlook 網頁版中建立及新增電子郵件簽名](https://support.office.com/article/5ff9dcfd-d3f1-447b-b2e9-39f91b074ea3)。