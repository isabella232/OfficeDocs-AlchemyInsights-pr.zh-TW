---
Title: 重新整理案例-並排顯示錯誤的疑難排解
ms.author: pebaum
author: pebaum
manager: mnirkhe
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "1331"
- "2500002"
ms.openlocfilehash: 568fa4662c7d312583348981f80319aaefcf7290
ms.sourcegitcommit: 1d98db8acb9959aba3b5e308a567ade6b62da56c
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/22/2019
ms.locfileid: "36506830"
---
# <a name="troubleshooting-refresh-scenarios---tile-errors"></a>重新整理案例-並排顯示錯誤的疑難排解

如果您遇到 Power BI 服務內的磚重新整理錯誤，請等候大約 10 到 15 分鐘的重新整理] 以反映在儀表板磚。 如果它仍未顯示，重新釘選到儀表板的視覺效果。

如需詳細資訊：[https://docs.microsoft.com/power-bi/refresh-troubleshooting-refresh-scenarios](https://docs.microsoft.com/power-bi/refresh-troubleshooting-refresh-scenarios)
