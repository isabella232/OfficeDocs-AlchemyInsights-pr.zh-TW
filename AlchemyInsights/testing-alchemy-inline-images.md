---
title: 測試魔力內嵌影像
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: a256e8680379c006287ae30996763051c4d9ff85
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/04/2019
ms.locfileid: "36743120"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="a9ceb-102">存取 [網域] 頁面</span><span class="sxs-lookup"><span data-stu-id="a9ceb-102">Access the Domains Page</span></span>

<span data-ttu-id="a9ceb-103">若要存取 [網域] 頁面上，只要按一下此圖示下方：</span><span class="sxs-lookup"><span data-stu-id="a9ceb-103">To access the domains page, just click this icon below:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="a9ceb-104">然後您可以新增、 修改或移除您的網域。</span><span class="sxs-lookup"><span data-stu-id="a9ceb-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="a9ceb-105">如需詳細資訊，請參閱[網域的常見問題集。](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span><span class="sxs-lookup"><span data-stu-id="a9ceb-105">For more information, see the [Domains FAQ.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span></span>
  